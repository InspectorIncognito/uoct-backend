...
class GeometryTestCase(BaseTestCase):
    def setUp(self):
        self.login_process()
        self.shape_data = {"name": "Test Shape"}
        self.segment_data = [
            {"sequence": 0, "geometry": [(0, 0), (0, 1), (0, 2)]},
            {"sequence": 1, "geometry": [(0, 3), (0, 4), (0, 5)]},
        ]
    ...
    def create_segments(self, segment_data=None):
        ...
        segments = Segment.objects.bulk_create(
            [Segment(**segment) for segment in segment_data]
        )
...
class ShapeTest(GeometryTestCase):
    ...
    def test_to_geojson(self):
        ...
        for segment in segments:
            simplified_linestring = shp_LineString(
...
class SegmentTest(GeometryTestCase):
    def test_create(self):
        segment_data = [
            {"sequence": 0, "geometry": [(0, 0), (0, 1), (0, 2)]},
            {"sequence": 1, "geometry": [(0, 3), (0, 4), (0, 5)]},
            {"sequence": 2, "geometry": [(0, 6), (0, 7), (0, 8)]},
        ]
...
class GeoJSONTest(GeometryTestCase):
    def test_shapes_to_geojson(self):
        ...
        expected = FeatureCollection(features=features)
